...
| `staffPrefs.fillOrder` | whose empty hours are offered first (`hannah, lissa, helen`; a technician not in the list — Zara and Beyhan today — is taken after them) |
| `workdays` on an operator line | the weekdays she works (0 = Pazar … 6 = Cumartesi); `rosterOn(date)` leaves her out on the others, so the gap-filler never offers Zara (`[1, 4]`) a Tuesday or Beyhan (`[2, 4]`) a Monday. Missing or malformed = every day (fails open) |
